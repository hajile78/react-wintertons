...
describe('createRoutes', () => {
  ...
  it('redirects anonymous users away from the role-protected post form', () => {
    renderRoute('/addPost')

    expect(screen.getByText('Login screen')).toBeInTheDocument()
    expect(screen.queryByText('Post form screen')).not.toBeInTheDocument()
  })

  it('renders the post form for authenticated users with the user role', () => {
    ...
    expect(mocks.hasRole).toHaveBeenCalledWith('user')
  })

  it('redirects authenticated users without the user role home', () => {
    mocks.isAuthenticated = true
    mocks.hasRole.mockReturnValue(false)

    renderRoute('/addPost')

    expect(screen.getByText('Posts screen')).toBeInTheDocument()
    expect(screen.queryByText('Post form screen')).not.toBeInTheDocument()
  })
